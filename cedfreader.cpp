...
std::string library_version(){
  ...
  version_info = edfapi::edf_get_version();
  return version_info;
}
...
edfapi::EDFFILE* safely_open_edf_file(std::string filename, int consistency, int loadevents, int loadsamples){
  ...
  if (ReturnValue != 0){
    ...
    std::fprintf(stderr, "%s", error_message_stream.str().c_str());
  }
...
void set_trial_navigation_up(edfapi::EDFFILE* edfFile, std::string start_marker_string, std::string end_marker_string){
  ...
  if (edf_set_trial_identifier(edfFile, start_marker_char, end_marker_char)){
    fprintf(stderr, "Error while setting up trial navigation identifier");
  }
...
void jump_to_trial(edfapi::EDFFILE* edfFile, int iTrial){
  if (edfapi::edf_jump_to_trial(edfFile, iTrial) != 0){
    ...
    fprintf(stderr, "%s", error_message_stream.str().c_str());
  }
}
...
std::vector<std::vector<double>> prepare_trial_headers(int total_trials){
  // row names
  std::vector<int64_t> row_index = std::vector<int64_t>(total_trials);
  ...
  // column names
  std::string strarr[] = {"trial", "duration", "starttime", "endtime",
  ...
  col_names.assign( {"trial", "duration", "starttime", "endtime",
			  "rec_time", "rec_sample_rate", "rec_eflags",
			  "rec_sflags", "rec_state", "rec_record_type",
			  "rec_pupil_type", "rec_recording_mode", "rec_filter_type",
			  "rec_pos_type", "rec_eye"} ); //strarr );
  
  // create the matrix
  std::vector< std::vector<double> > trial_headers(total_trials); //= NumericMatrix(total_trials, col_names.size());
  for( size_t x=0; x<trial_headers.size(); ++x )
    {
      auto tmp = std::vector<double>(col_names.size() );
      trial_headers[x] = tmp;
    }
...
void read_trial_header(edfapi::EDFFILE* edfFile, std::vector<std::vector<double>> &trial_headers, int iTrial){
...
void append_event(TRIAL_EVENTS &events, edfapi::FEVENT new_event, unsigned int iTrial, edfapi::UINT32 trial_start){
  ...
  if (message_ptr == 0 || message_ptr == NULL){
    ...
    std::strncpy(message_char, &(message_ptr->c), message_ptr->len);
...
//' @keywords internal
void append_sample(TRIAL_SAMPLES &samples, edfapi::FSAMPLE new_sample, unsigned int iTrial, edfapi::UINT32 trial_start, std::vector<bool> sample_attr_flag)
{
...
//[[Rcpp::export]]
void read_edf_file(std::string filename,
                   int consistency,
                   bool import_events,
                   bool import_recordings,
                   bool import_samples,
                   std::vector<bool> sample_attr_flag,
		   ...
		   TRIAL_RECORDINGS& all_recordings)
{
  ...
  if (verbose){
    std::fprintf(stderr, "Trials count: %d\n", total_trials);
  }

  // creating headers, events, and samples
  auto trial_headers = prepare_trial_headers(total_trials);
  ...
  for(unsigned int iTrial = 0; iTrial< total_trials; iTrial++){
    ...
    if (trial_end_time <= trial_start_time){
      std::fprintf(stderr, "Skipping trial %d due to zero or negative duration.", iTrial+1);
      continue;
    }
...
void convert_to_csv( std::string edffname,
		     std::string samplesfpath,
		     std::string eventsfpath)
{
  TRIAL_EVENTS all_events;
  TRIAL_SAMPLES all_samples;
  TRIAL_RECORDINGS all_recordings;
  std::vector<bool> attr(32, true);
  
  read_edf_file(edffname,
		0, //int consistency,
                true, //   bool import_events,
                true, //   bool import_recordings,
                true, //   bool import_samples,
		attr, //std::vector<bool> sample_attr_flag,
		std::string(), //std::string start_marker_string,
		std::string(), //std::string end_marker_string,
		false, //bool verbose,
		all_events,
		all_samples,
		all_recordings);
  
  auto outfs = std::ofstream(samplesfpath);
  auto s = all_samples;
  size_t len = s.trial_index.size();
  size_t tlen = s.time.size();
  if( len != tlen )
    {
      std::fprintf(stderr, "Whoops time len not correct\n");
      exit(1);
    }
  std::string sep=",";
  outfs <<
...
int main(int argc, char** argv)
{
  ...
  std::string oute = infile + "_" + tag + "_events.csv";
  std::fprintf(stdout, "REV: RUNNING! (in=%s)\n", infile.c_str());
  convert_to_csv(infile, outs, oute);
  return 0;
};
